...
volatile unsigned long last_capture = 0; // Store the last capture value
volatile unsigned int prescaling_factor = 1;
extern volatile float frequency;
extern bool state; 
extern volatile float set_frequency;
extern bool pulseFlag;
uint32_t i = 0, j = 0;
...
void TIM1_CH1_CCP_IRQHandler(void) 
{
  ...
  if (state == 1 && pulseFlag == 1) {
		GPIO_WriteHigh(GPIOC, GPIO_PIN_2); // Send pulse
		for(i = 0; i < 1030; i++)
		{
