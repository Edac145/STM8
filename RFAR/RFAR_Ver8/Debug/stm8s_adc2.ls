...
1235                     ; 311 }
1238  0153 87            	retf
1277                     ; 320 uint16_t ADC2_GetConversionValue(void)
1277                     ; 321 {
1278                     	switch	.text
1279  0154               f_ADC2_GetConversionValue:
1281  0154 5205          	subw	sp,#5
1282       00000005      OFST:	set	5
1285                     ; 322   uint16_t temph = 0;
1287                     ; 323   uint8_t templ = 0;
1289                     ; 325   if ((ADC2->CR2 & ADC2_CR2_ALIGN) != 0) /* Right alignment */
1291  0156 c65402        	ld	a,21506
1292  0159 a508          	bcp	a,#8
1293  015b 2715          	jreq	L725
1294                     ; 328     templ = ADC2->DRL;
1296  015d c65405        	ld	a,21509
1297  0160 6b03          	ld	(OFST-2,sp),a
1299                     ; 330     temph = ADC2->DRH;
1301  0162 c65404        	ld	a,21508
1302  0165 5f            	clrw	x
1303  0166 97            	ld	xl,a
1304  0167 1f04          	ldw	(OFST-1,sp),x
1306                     ; 332     temph = (uint16_t)(templ | (uint16_t)(temph << (uint8_t)8));
1308  0169 1e04          	ldw	x,(OFST-1,sp)
1309  016b 7b03          	ld	a,(OFST-2,sp)
1310  016d 02            	rlwa	x,a
1311  016e 1f04          	ldw	(OFST-1,sp),x
1314  0170 2021          	jra	L135
1315  0172               L725:
1316                     ; 337     temph = ADC2->DRH;
1318  0172 c65404        	ld	a,21508
1319  0175 5f            	clrw	x
1320  0176 97            	ld	xl,a
1321  0177 1f04          	ldw	(OFST-1,sp),x
1323                     ; 339     templ = ADC2->DRL;
1325  0179 c65405        	ld	a,21509
1326  017c 6b03          	ld	(OFST-2,sp),a
1328                     ; 341     temph = (uint16_t)((uint16_t)((uint16_t)templ << 6) | (uint16_t)((uint16_t)temph << 8));
1330  017e 1e04          	ldw	x,(OFST-1,sp)
1331  0180 4f            	clr	a
1332  0181 02            	rlwa	x,a
1333  0182 1f01          	ldw	(OFST-4,sp),x
1335  0184 7b03          	ld	a,(OFST-2,sp)
1336  0186 97            	ld	xl,a
1337  0187 a640          	ld	a,#64
1338  0189 42            	mul	x,a
1339  018a 01            	rrwa	x,a
1340  018b 1a02          	or	a,(OFST-3,sp)
1341  018d 01            	rrwa	x,a
1342  018e 1a01          	or	a,(OFST-4,sp)
1343  0190 01            	rrwa	x,a
1344  0191 1f04          	ldw	(OFST-1,sp),x
1346  0193               L135:
1347                     ; 344   return ((uint16_t)temph);
1349  0193 1e04          	ldw	x,(OFST-1,sp)
1352  0195 5b05          	addw	sp,#5
1353  0197 87            	retf
1396                     ; 352 FlagStatus ADC2_GetFlagStatus(void)
1396                     ; 353 {
1397                     	switch	.text
1398  0198               f_ADC2_GetFlagStatus:
1402                     ; 355   return (FlagStatus)(ADC2->CSR & ADC2_CSR_EOC);
1404  0198 c65400        	ld	a,21504
1405  019b a480          	and	a,#128
1408  019d 87            	retf
1430                     ; 363 void ADC2_ClearFlag(void)
1430                     ; 364 {
1431                     	switch	.text
1432  019e               f_ADC2_ClearFlag:
1436                     ; 365   ADC2->CSR &= (uint8_t)(~ADC2_CSR_EOC);
1438  019e 721f5400      	bres	21504,#7
1439                     ; 366 }
1442  01a2 87            	retf
1465                     ; 374 ITStatus ADC2_GetITStatus(void)
1465                     ; 375 {
1466                     	switch	.text
1467  01a3               f_ADC2_GetITStatus:
1471                     ; 376   return (ITStatus)(ADC2->CSR & ADC2_CSR_EOC);
1473  01a3 c65400        	ld	a,21504
1474  01a6 a480          	and	a,#128
1477  01a8 87            	retf
1500                     ; 384 void ADC2_ClearITPendingBit(void)
1500                     ; 385 {
1501                     	switch	.text
1502  01a9               f_ADC2_ClearITPendingBit:
1506                     ; 386   ADC2->CSR &= (uint8_t)(~ADC2_CSR_EOC);
1508  01a9 721f5400      	bres	21504,#7
1509                     ; 387 }
1512  01ad 87            	retf
1524                     	xdef	f_ADC2_ClearITPendingBit
1525                     	xdef	f_ADC2_GetITStatus
1526                     	xdef	f_ADC2_ClearFlag
1527                     	xdef	f_ADC2_GetFlagStatus
1528                     	xdef	f_ADC2_GetConversionValue
1529                     	xdef	f_ADC2_StartConversion
1530                     	xdef	f_ADC2_ExternalTriggerConfig
1531                     	xdef	f_ADC2_ConversionConfig
1532                     	xdef	f_ADC2_SchmittTriggerConfig
1533                     	xdef	f_ADC2_PrescalerConfig
1534                     	xdef	f_ADC2_ITConfig
1535                     	xdef	f_ADC2_Cmd
1536                     	xdef	f_ADC2_Init
1537                     	xdef	f_ADC2_DeInit
1556                     	end
