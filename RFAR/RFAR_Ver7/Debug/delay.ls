 ...
 157  0054 80            	iret
 200                     ; 47 void delay_ms(uint16_t ms)
 200                     ; 48 {
 201                     	switch	.text
 202  0055               f_delay_ms:
 204  0055 89            	pushw	x
 ...
 240  0072 87            	retf
 302                     ; 58 void delay_us(uint16_t us)
 302                     ; 59 {
 303                     	switch	.text
 304  0073               f_delay_us:
 306  0073 89            	pushw	x
 ...
 311                     ; 60 	uint8_t start_us = TIM4_GetCounter();  //tim4 increments every us
 313  0076 8d000000      	callf	f_TIM4_GetCounter
 315  007a 6b05          	ld	(OFST+0,sp),a
 317                     ; 61 	if(us>=250){   //we only need to bother with the following for delays greater than 1 tick (250us)
 319  007c 1e06          	ldw	x,(OFST+1,sp)
 320  007e a300fa        	cpw	x,#250
 321  0081 2514          	jrult	L131
 322                     ; 62 		uint16_t start_tick = (uint16_t)tick;  //the tick increments every 250us
 324  0083 be02          	ldw	x,_tick+2
 325  0085 1f01          	ldw	(OFST-4,sp),x
 327                     ; 63 		uint16_t delay_ticks = us/250;
 329  0087 1e06          	ldw	x,(OFST+1,sp)
 330  0089 a6fa          	ld	a,#250
 331  008b 62            	div	x,a
 332  008c 1f03          	ldw	(OFST-2,sp),x
 335  008e               L321:
 336                     ; 65 		while(((uint16_t)tick - start_tick) < delay_ticks); // delay in multiples of 250us
 338  008e be02          	ldw	x,_tick+2
 339  0090 72f001        	subw	x,(OFST-4,sp)
 340  0093 1303          	cpw	x,(OFST-2,sp)
 341  0095 25f7          	jrult	L321
 342  0097               L131:
 343                     ; 67 	while(TIM4_GetCounter() < start_us); //now wait until our 1us counter matches our start us
 ...
 353  00a1 87            	retf
 376                     ; 71 uint16_t millis(void){
 377                     	switch	.text
 378  00a2               f_millis:
 382                     ; 72 	return((uint16_t)(tick >> 2)); // divide tick by 4 returns milliseconds
 ...
 393  00b1 87            	retf
 417                     ; 76 uint32_t micros(void){
 418                     	switch	.text
 419  00b2               f_micros:
 423                     ; 77 	return(tick*250 + TIM4_GetCounter()); //each tick is worth 250us
