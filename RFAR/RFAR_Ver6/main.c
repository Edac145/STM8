...
float process_FDR_signal(void) {
    ...
    while (1) {
        current_amplitude = process_adc_signal(FDR_SIGNAL, NULL, &current_amplitude);

        if (isThyristorON) {
            send_square_pulse(3000);
            GPIO_WriteHigh(GPIOD, GPIO_PIN_0); // Turn on LED ORANGE
						//printf("Commutation Thyristor ON");
        } else {
            GPIO_WriteLow(GPIOD, GPIO_PIN_0);
        }
    }
    return current_amplitude; // In practice, this is unreachable due to the infinite loop
}
